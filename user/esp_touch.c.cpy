...
// In case of the mesh-network, ESP-TOUCH is used to connect a not yet connected device to any desired router,
// thus initializing a new network.
//
...
// Callback-function, that is executed on successful establishing a connection to the router via ESP-TOUCH
static void ICACHE_FLASH_ATTR esptouch_success_cb(void *arg) {
  os_printf("esptouch_success_cb: Success! Stopping ESP-TOUCH now!\n");

  // Disarm and free the timeout-timer and stop the smartconfiguration-mode
  esptouch_disable();

  // Set the WiFi-operation-mode to STATIONAP_MODE to be able to act as the mesh-network's root-device
  wifi_set_opmode(STATIONAP_MODE);

  esptouch_success = true;
}

// Callback-function, that is executed on a timeout; print out the current status and restart ESP-TOUCH until
// the defined limit of attempts has been reached
static void ICACHE_FLASH_ATTR esptouch_fail_cb(void *arg) {
  ...
  if (esptouch_attempt_count < ESP_TOUCH_ATTEMPTS_LIMIT) {
    ...
    // Initialize and arm the timer that executes the timeout-callback, if no configuration-packages are received
    // until the defined threshold (cf. ESP_TOUCH_CONFIG_TIMEOUT_THRESHOLD)
    ...
    // Restart ESP-TOUCH
    extern void esptouch_status_cb(sc_status status, void *arg);  // Has to be declared seperately here as an extern function because the compiler resolves the scope top-down
...
// Callback-function, that is executed on the start of ESP-TOUCH; increase the attempt-count, print out the
// current status
...
void ICACHE_FLASH_ATTR esptouch_status_cb(sc_status status, void *arg) {
  switch (status) {
    ...
    case SC_STATUS_GETTING_SSID_PSWD:
      ...
      // Arm the timer that executes the timeout-callback, if the station-configuration couldn't be obtained
      // until the defined threshold (cf. ESP_TOUCH_RECV_TIMEOUT_THRESHOLD)
      ...
      break;
    // Connecting to the router whose SSID and password have been obtained from the intermediary-device
    case SC_STATUS_LINK:
      ...
      // Set the obtained SSID and password as mesh-router-configuration and try to connect to the access-point
      wifi_station_disconnect();  // Clear all currently established connections
      if (espconn_mesh_set_router(station_conf) && wifi_station_set_config(station_conf)) {
      ...
      // Arm the timer that executes the timeout-callback, if no connection to the router could be established
      // until the defined threshold (cf. ESP_TOUCH_CONNECTION_TIMEOUT_THRESHOLD)
      if (esptouch_timeout_timer && esptouch_func.esptouch_fail_cb) {
        os_timer_disarm(esptouch_timeout_timer);
        os_timer_setfn(esptouch_timeout_timer, esptouch_func.esptouch_fail_cb, NULL);
      ...
      break;
    // Connection successfully established; stopping smartconfiguration-mode and executing success-callback
...
void ICACHE_FLASH_ATTR esptouch_init(void) {
  ...
  // Set ESP-TOUCH to running and not (yet) successful and initialize the attempt-count
  ...
  // Set WiFi to station mode to be able to receive the router's SSID and password from the intermediary-device
  os_printf("esptouch_init: Set WiFi to station mode!\n");
  wifi_set_opmode(STATION_MODE);

  // Assign callback-functions and set the smartconfiguration-type to SC_TYPE_ESPTOUCH
  ...
  if (!esptouch_timeout_timer) {
    os_printf("esptouch_init: Failed to initialize the timeout-timer! Continuing without!\n");
  }

  // Configure and arm the timer that executes the timeout-callback, if no configuration-packages are received
  // until the defined threshold (cf. ESP_TOUCH_CONFIG_TIMEOUT_THRESHOLD)
  if (esptouch_timeout_timer && esptouch_func.esptouch_fail_cb) {
    os_timer_disarm(esptouch_timeout_timer);
    os_timer_setfn(esptouch_timeout_timer, esptouch_func.esptouch_fail_cb, NULL);
  ...
  if (esptouch_status_cb) {
    ...
    os_printf("esptouch_init: Failed to start smartconfiguration-mode! Invalid status-callback-function!\n");
  }
}
